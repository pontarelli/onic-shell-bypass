...
module pcap_parse #(
    ...
    task readPacketHeader();
        begin
            ...
            $display("PCAP source:  packet %0d: incl_length %0d orig_length %0d", pktcount, pktSz, diskSz );
        end
    ...
    assign can_advance_output = ~pause && ~pause_ifg && (~valid || ready);
    ...
    always @(posedge clk) begin
        ...
        if (eof != 0) begin
            ...
            if (can_advance_output) begin
                ...
                // Hold output beat while stalled/paused.
                /*if(ready) begin
                    valid <= 0;
                end*/
            end
