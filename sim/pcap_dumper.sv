...
module pcap_dumper #(
    ...
    input  wire               [47:0] tuser   ,
    output wire                      tready  ,
    input  wire                      tvalid  ,
    input  wire                      tlast   ,
    input  wire                      eos     , // End of stream (close the file)
    output reg  [               15:0] pktcount // nonba edit
);
...
//assign tready = (randomNumber %2)==0 ? 1'b0 : 1'b1;
assign tready = 1'b1;
    ...
    always_ff @(negedge rst_n or posedge clk) begin
        if(~rst_n) begin
            ...
            if(tvalid && tready) begin
                ...
                if ((tuser[15:0] != current_tuser)&& !flag_new_packet) begin 
                     $display("ERROR PCAP sink:  packet %0d:  tuser changed from %0d to %d", pktcount, current_tuser , tuser[15:0] );
                end    
                current_tuser = tuser[15:0];
                flag_new_packet =1'b0;
                if(tlast) begin
                    flag_new_packet =1'b1;
                    diskSz        = c_caplen == 0 ? pktSz : pktSz<c_caplen ? pktSz : c_caplen;
                    $display("PCAP sink:  packet %0d: incl_length %0d orig_length %0d", pktcount, diskSz, pktSz  );
                    pktcount++;
                    ...
                    if (tuser[15:0] != pktSz) begin 
                        $display("ERROR  PCAP sink:  packet %0d:  error in pkt len packet size= %0d tuser_size=%d", pktcount, pktSz, tuser[15:0] );
                        end
                    pktSz          = 0;
                end
            end
